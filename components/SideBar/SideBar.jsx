...
//               <img src={postImage || "./uplouder.svg"} alt="add_posts" className="upload-icon" />
//               <input
//                 type="file"
//                 accept="image/*"
...
function Sidebar({ onCreatePost }) {
  ...
  const [postText, setPostText] = useState(""); // Текст поста
  ...
  const handleImageUpload = (event) => {
    const file = event.target.files[0];
    if (file) {
      setPostImage(URL.createObjectURL(file)); // Создаем URL для загруженного файла
    }
  };
  ...
  const handleCommentSubmit = (event) => {
    event.preventDefault(); // Предотвращаем перезагрузку страницы
    setComments([...comments, { text: postText, date: new Date() }]);
    setPostText(""); // Очищаем поле текста
  };
      ...
      <ul className="menu">
        {["Home", "Search", "Explore", "Messages", "Notifications", "Create", "Profile"].map((item) => (
         ...
         { item !== "Create" ?  <Link
            ...
            </Link> : <div onClick={()=> setShowModal(true)} className={`menu-item-container ${activeTab === item ? "active" : ""}`}>
